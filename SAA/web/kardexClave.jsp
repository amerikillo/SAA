...
<%
    ...
    try {
        /**
         * Se obtiene la clave a partir del CB
         */
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                                ...
                                <%
                                    try {
                                        if (!request.getParameter("Lote").equals("")) {
                                            /**
                                             * Boton para generar trazabilidad
                                             */
                                %>
                    ...
                    <%
                        try {
                            /**
                             * Cálculo de total de existencias por clave
                             */
                            con.conectar();
                            ...
                            <tbody>
                                <%
                                    ...
                                    try {
                                        con.conectar();
                                        /**
                                         * Obtención de los movimientos
                                         * distintos a 1000, ya que son de
                                         * entradas y salidass
                                         */
                                        ResultSet rset = con.consulta("select m.F_User, m.F_ConMov, c.F_DesCon, m.F_ProMov, l.F_ClaLot,  DATE_FORMAT(l.F_FecCad, '%d/%m/%Y'), (m.F_CantMov*m.F_SigMov) as pzs, m.F_CostMov, u.F_DesUbi, DATE_FORMAT(m.F_FecMov, '%d/%m/%Y'), m.F_hora, m.F_DocMov, m.F_IdMov FROM tb_movinv m, tb_coninv c, tb_ubica u, tb_lote l WHERE m.F_ConMov = c.F_IdCon AND m.F_UbiMov = u.F_ClaUbi AND m.F_LotMov = l.F_FolLot and m.F_ProMov = '" + Clave + "' and l.F_ClaLot ='" + request.getParameter("Lote") + "' and l.F_FecCad=STR_TO_Date('" + request.getParameter("Cadu") + "', '%d/%m/%Y') and m.F_ConMov!=1000 GROUP BY m.F_IdMov ORDER BY m.F_IdMov");
                                        while (rset.next()) {
                                            /**
                                             * Se obtiene la orden de compra en
                                             * caso se de compra (1) y proveedor
                                             */
                                            ...
                                            if (rset.getString(2).equals("51")) {
                                                /**
                                                 * Si el concepto es 51 será
                                                 * salida por remisión y se
                                                 * obtiene el dato
                                                 */
                                             ...
                                             */
                                            sumatoria = sumatoria + rset.getInt("pzs");
                                %>
                                <tr>
                                    ...
                                    <td><%=formatter.format(sumatoria)%></td>
                        ...
                        <table class="table table-bordered table-striped" width="100%" id="kardexTab">
                            ...
                            <tbody>
                                <%
                                    try {
                                        /**
                                         * Redistribuciones entre racks del
                                         * cedis, es para movimientos 1000
                                         */
                                        con.conectar();
