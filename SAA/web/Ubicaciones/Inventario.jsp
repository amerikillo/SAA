        ...
        <div class="container">
            ...
            <div class="container">               
                    <table class="table">
                        <tbody>                            
                            <tr>
                                <th>Clave:</th><td><input type="text" id="clave" name="clave" placeholder="" readonly="" value="<%//=clave%>" class="text-center" />&nbsp;&nbsp;
                                   <select id="selectc">                                       
                                        <option>--Clave--</option>
                                        <%
                                        if (CB !=""){    
                            ...
                            <tr>
                                <th>Lote</th><td><input type="text" id="lote" name="lote" placeholder="" class="text-center" value="<%//=lote%>"/>&nbsp;&nbsp;      

                                    <select id="selectl">
                                        <option id="op">--Lote--</option>
                                        <%
                                        if (CB !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_ClaLot FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi WHERE U.F_Cb='"+CB+"' GROUP BY L.F_ClaLot");
                                        }else if (CBM !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_ClaLot FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi WHERE L.F_Cb='"+CBM+"' GROUP BY L.F_ClaLot");    
                                        }
                                        while(Rset_Clave.next()){
                                       %>
                                        <option value="<%=Rset_Clave.getString(1)%>"><%=Rset_Clave.getString(1)%></option>
                            ...
                            <tr>
                                <th>Caducidad</th><td><input type="text" id="caducidad" name="caducidad" readonly="" placeholder="" class="text-center" value="<%//=caducidad%>"/>&nbsp;<label class="icon-calendar icon-2x"></label>&nbsp;&nbsp;<select id="selectCadu">
                                        <option id="op">--Caducidad--</option>
                                        <%
                                        if (CB !=""){
                                        Rset_Clave = con.consulta("SELECT DATE_FORMAT(L.F_FecCad,'%d/%m/%Y') AS Fecha FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi WHERE U.F_Cb='"+CB+"' GROUP BY L.F_FecCad");
                                        }else if (CBM !=""){
                                        Rset_Clave = con.consulta("SELECT DATE_FORMAT(L.F_FecCad,'%d/%m/%Y') AS Fecha FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi WHERE L.F_Cb='"+CBM+"' GROUP BY L.F_FecCad");    
                                        }
                                        while(Rset_Clave.next()){
                                       %>
                                        <option value="<%=Rset_Clave.getString(1)%>"><%=Rset_Clave.getString(1)%></option>
                            ...
                            <tr>
                                <th>Proveedor</th><td><input type="text" id="proveedor" name="proveedor" placeholder="" readonly="" class="text-center" value="<%//=proveedor%>"/>&nbsp;&nbsp;<select id="selectProv">
                                        <option id="op">--Proveedor--</option>
                                        <%
                                        if (CB !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_ClaOrg,P.F_NomPro FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi INNER JOIN tb_proveedor P ON L.F_ClaOrg=P.F_ClaProve WHERE U.F_Cb='"+CB+"' GROUP BY L.F_ClaOrg");
                                        }else if (CBM !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_ClaOrg,P.F_NomPro FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi INNER JOIN tb_proveedor P ON L.F_ClaOrg=P.F_ClaProve WHERE L.F_Cb='"+CBM+"' GROUP BY L.F_ClaOrg");    
                                        }
                                        while(Rset_Clave.next()){
                                       %>
                                        <option value="<%=Rset_Clave.getString(1)%>"><%=Rset_Clave.getString(2)%></option>
                                        <%}%>
                                    </select></td>
                            </tr>
                            <tr>
                                <%
                                if (CBM !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_Cb FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi WHERE L.F_Cb='"+CBM+"' GROUP BY L.F_Cb");    
                                        while(Rset_Clave.next()){
                                            cbm = Rset_Clave.getString("L.F_Cb");
                                        }
                                %>
                                <th>CB</th><td><input type="text" id="cb" name="cb" placeholder="" class="text-center" value="<%=cbm%>"/>&nbsp;&nbsp;
                                        <%}else{%>
                               <th>CB</th><td><input type="text" id="cb" name="cb" placeholder="" class="text-center" value="<%//=cbm%>"/>&nbsp;&nbsp;
                                        <%}%>
                                   <select id="selectCb">
                                        ...
                                        <%
                                        if (CB !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_Cb FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi WHERE U.F_Cb='"+CB+"' GROUP BY L.F_Cb");
                                        }else if (CBM !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_Cb FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi WHERE L.F_Cb='"+CBM+"' GROUP BY L.F_Cb");    
                                        }
                                        while(Rset_Clave.next()){
                            ...
                            <tr>
                                <th>Marca</th><td><input type="text" id="marca" name="marca" placeholder="" readonly="" class="text-center" value="<%//=marca%>"/>&nbsp;&nbsp;<select id="selectMarca">
                                        <option id="op">--Marca--</option>
                                        <%
                                        if (CB !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_ClaMar,M.F_DesMar FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi INNER JOIN tb_marca M ON L.F_ClaMar=M.F_ClaMar WHERE U.F_Cb='"+CB+"' GROUP BY L.F_ClaMar");
                                        }else if (CBM !=""){
                                        Rset_Clave = con.consulta("SELECT L.F_ClaMar,M.F_DesMar FROM tb_lote L INNER JOIN tb_ubica U on L.F_Ubica=U.F_ClaUbi INNER JOIN tb_marca M ON L.F_ClaMar=M.F_ClaMar WHERE L.F_Cb='"+CBM+"' GROUP BY L.F_ClaMar");    
                                        }
                                        while(Rset_Clave.next()){
                                       %>
                                        <option value="<%=Rset_Clave.getString(1)%>"><%=Rset_Clave.getString(2)%></option>
                            ...
                            <tr>
                                <th>Piezas x Cajas</th><td><input type="text" id="pzc" name="pzc" value="<%//=pzas%>" placeholder="" class="text-center"/></td>
                            </tr>  
                            <tr>
                                <th>Cantidad</th><td><input type="text" id="restom" name="restom" placeholder="" class="text-center" value="<%//=cantidad%>" /></td>
                            </tr>                            
                        </tbody>
           ...
           if (btn.equals("Agregar")){
               ...
               if (cont > 0){
                   ...
                   con.actualizar("INSERT INTO tb_lote VALUES(0,'"+clave+"','"+lote+"','"+caducidad+"','"+cantidad+"','"+pzas+"','"+proveedor+"','"+ubica+"',CURDATE(),'"+cbm+"','"+marca+"')");
               }
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#btn-agregar").click(function() {
                ...
                if (pzs == "") {
                    missinginfo += "\n El campo Piezas x Cajas no debe estar Vacío.";
                }
                if (lote == "") {
